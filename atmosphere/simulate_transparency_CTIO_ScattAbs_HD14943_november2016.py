...
h2ofile='info/CTIO_atm/AIRS_2016_TotH2OVap_D.csv'	
o3file='info/CTIO_atm/AIRS_2016_TotO3_D.csv'

source_name='HD14943'

CTIO_Altitude = 2.215   # in k meters from astropy package (Cerro Pachon)
...
TOPDIR='simulations/RT/2.0/CT'
#logdatafile='info/LogBook/2016/08/logbk_HD205905.fits'
logdatafile='info/LogBook/2016/11/logbk_HD14943_ctioNov2016.fits'

selected_dates = ['2016-10-01']
...
if __name__ == "__main__":
    ...
    selected_dates=np.array(selected_dates)
    ...
    if not np.array_equal(dates_data_o3,dates_data_h2o):
        print 'dates for O3 and H2O are not equal. Stop ! '
        os.exit()
    ...
    for date in selected_dates:
        idx=np.where(dates_data_o3==date)
        list_of_month_sel.append(idx)
    list_of_month_sel=np.array(list_of_month_sel)
    list_of_month_sel=list_of_month_sel.flatten()
    
    
    selected_data_month_aver_o3=data_month_aver_o3[list_of_month_sel]
    selected_avo3=selected_data_month_aver_o3.mean()
    selected_data_month_sdev_o3=data_month_sdev_o3[list_of_month_sel]
    semected_data_month_ct_o3=data_month_ct_o3[list_of_month_sel]
    selected_data_month_err_o3=data_month_err_o3[list_of_month_sel]
    selected_data_month_p1std_o3=selected_data_month_aver_o3+selected_data_month_sdev_o3
    selected_data_month_m1std_o3=selected_data_month_aver_o3-selected_data_month_sdev_o3
    selected_data_month_min_o3=data_month_min_o3[list_of_month_sel]
    selected_data_month_max_o3=data_month_max_o3[list_of_month_sel]
    
    
    selected_data_month_aver_h2o=data_month_aver_h2o[list_of_month_sel]
    avh2o=data_month_aver_h2o.mean()
    selected_data_month_sdev_h2o=data_month_sdev_h2o[list_of_month_sel]
    semected_data_month_ct_h2o=data_month_ct_h2o[list_of_month_sel]
    selected_data_month_err_h2o=data_month_err_h2o[list_of_month_sel]
    selected_data_month_p1std_h2o=selected_data_month_aver_h2o+selected_data_month_sdev_h2o
    selected_data_month_m1std_h2o=selected_data_month_aver_h2o-selected_data_month_sdev_h2o
    selected_data_month_min_h2o=data_month_min_h2o[list_of_month_sel]
    selected_data_month_max_h2o=data_month_max_h2o[list_of_month_sel]

    # airmass indexes defined for the whole production from z=1 to z=3
    #----------------------------------------
    
    
    hdulist = fits.open(logdatafile)
    event_number=hdulist[1].data['number']
    event_airmass=hdulist[1].data['airmass']
    hdulist.close()
    
    airmasses_indexes=event_number
...
#   Selection of absorption model 
    ...
    for atmosphere in theatmospheres:
        ...
        for idx,the_date in np.ndenumerate(selected_dates):
            print idx
            idx1=idx[0]
...
#           ozon_str='O3 '+str(ozon_val)+ ' DU'          
            ...
            for variation in atm_variations:
               if variation == 'aver':
                   ozon_val=selected_data_month_aver_o3[idx1]
                   pwv_val=selected_data_month_aver_h2o[idx1]
                   
               elif variation == 'std1':
                   ozon_val=selected_data_month_p1std_o3[idx1]
                   pwv_val=selected_data_month_p1std_h2o[idx1]                  
               elif variation == 'std2': 
                   ozon_val=selected_data_month_m1std_o3[idx1]
                   pwv_val=selected_data_month_m1std_h2o[idx1]            
               elif variation == 'min': 
                   ozon_val=selected_data_month_min_o3[idx1]
                   pwv_val=selected_data_month_min_h2o[idx1]                   
               elif variation == 'max':
                   ozon_val=selected_data_month_max_o3[idx1]
                   pwv_val=selected_data_month_max_h2o[idx1]
               else:
                   print "Bad Variation"
                   os._exit(1)
           ...
           # 4) LOOP ON AIRMASSES 
               for index,amfileindex in np.ndenumerate(airmasses_indexes):
                    ...
                    airmass=event_airmass[index]
                    
            
                    BaseFilename=BaseFilename_part1+atmkey+'_'+Proc+'_'+Mod+'_z'+str(amfileindex)+'_'+the_year_str+'_'+the_month_str+'_'+variation
                    BaseFilename=BaseFilename_part1+atmkey+'_'+Proc+'_'+Mod+'_'+source_name+'_Nev'+str(amfileindex)+'_'+the_year_str+'_'+the_month_str+'_'+variation                    
